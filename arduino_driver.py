...
class Arduino:
  ...
  #      self.execute_ack(cmd)  
    ...
    #   left_ticks_per_loop = int(left_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        left_ticks_per_loop = int(left_revs_per_second * self.encoder_resolution / self.PID_RATE * self.gear_reduction)
    #   right_ticks_per_loop  = int(right_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        right_ticks_per_loop  = int(right_revs_per_second * self.encoder_resolution / self.PID_RATE * self.gear_reduction)
    #     self.PID_RATE = 30 # Do not change this!  It is a fixed property of the Arduino PID controller.
    #     self.PID_INTERVAL = 1000 / 30
